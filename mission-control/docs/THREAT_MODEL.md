...
| R-008 | Supply-chain compromise | T/E | critical | lockfile, dependency audits, migration checks, minimal non-root container, read-only filesystem, retained SPDX SBOM, exact-image high/critical vulnerability gate, commit-bound image receipt | medium | signed image provenance, immutable registry promotion, and admission verification in CI/CD |
